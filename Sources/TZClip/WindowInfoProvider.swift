...
class WindowInfoProvider {
    ...
    func captureWindows() async -> Bool {
        do {
            ...
            self.isReady = true
            // print("Captured \(self.windows.count) windows.")
            return true
    ...
    func window(at point: NSPoint) -> DetectedWindow? {
        ...
        // windows array is ordered Front-to-Back.
        let candidates = windows.filter { $0.frame.contains(point) }
        
        guard let topMost = candidates.first else {
            return nil
        }
        ...
        let sameAppWindows = candidates.filter { 
