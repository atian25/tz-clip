...
- [x] **文字工具增强**:
    - [x] 支持描边、字体选择、所见即所得编辑。
    - [x] 优化编辑状态隔离与连续输入体验。
    - [x] 优化序号标注与文字缩放效果（比例联动：数字 0.6× 且上限 20、徽章冻结；文字 12–100pt）

### 4.3 贴图 (Phase 4: Pin)
- [ ] 将截图固定在屏幕顶层。

### 4.4 命令与控制器收敛
- [ ] 将选区创建/移动/缩放接口从视图收敛到 `SelectionController`，统一驱动布局与属性
- [ ] `CommandBus` 打通 Pin/OCR/滚动截屏的占位命令与服务实现
- [ ] 属性面板：线条类端点/箭头样式、字体列表本地化与常用缓存
- [ ] 撤销栈容量与性能策略评估，增加基准测试

### M1 更新摘要（近期变更）
- 命令与撤销：扩展 `redo/delete`，集中分发到 `CommandBus`；`redoStack` 支持 `⌘⇧Z`
- 交互与防误触：形状创建采用拖拽阈值；文本/序号单击创建后，若未切换工具则下一次空白点击不创建；切换工具后立即允许创建
- 属性面板与布局：文本/序号第三块始终显示；线条类自适应收窄；布局由 `ToolbarLayoutService` 计算
- 字号与序号规范：文本/序号 12–100pt；序号数字 `min(lineWidth × 0.6, 20)`；数字达上限后徽章冻结
