...
#### 2.2.2 自适应布局规则
* 右侧区宽度按可见控件的实际内容动态计算，最小 176；避免覆盖左侧“大小/不透明度”与中间颜色矩阵。
* 左侧“大小/不透明度”的数值标签宽度按最大可能字符串测量：形状使用 `20px`，文字/序号使用 `100pt`；滑块宽度随之缩放，确保不被遮挡。
* 颜色矩阵采用固定 3×2 栅格宽度计算分隔线位置，右侧区整体仅水平重排，保持两行的垂直基线稳定。
* 面板整体摆放使用视图真实尺寸进行避让（`fittingSize`），在工具栏上下自动切换，避免互相遮盖。
...
*   **绘制 (Creating)**:
    ...
    *   **文字工具**:
        ...
        *   **点击保护**: 仅在刚创建一段文字且未切换工具时，抑制下一次空白点击的再次创建；一旦在工具栏切换到任意工具，立即允许创建；仅调整属性不会解除保护。
    *   **序号工具 (Counter)**:
        ...
        *   **点击保护**: 规则与文字工具一致：刚创建且未切换工具时抑制下一次空白点击创建；切换工具后允许创建；仅调整属性不会解除保护。
...
#### 2.3.1 中文输入法合成阶段
* 在输入法合成（拼音/候选）阶段，编辑框按“预计文本”即时测量并扩宽，避免先换行后再变宽的闪烁。
* 文本容器禁用因容器宽度导致的自动换行（`widthTracksTextView=false`、`containerSize.width=∞`、`lineBreakMode=byClipping`）。
* 行高与最终绘制一致：使用 `ascender-descender` 固定行高，减少编辑态与保存后的尺寸差异。
    *   **序号编辑**:
        *   **拖拽移动**:
