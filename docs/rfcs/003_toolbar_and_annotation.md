...
- 命中测试原则
  - 填充形状：内部点命中（矩形/椭圆）。
  - 非填充形状：使用加粗描边命中，提升容错（线段/箭头/轮廓）；默认命中容差建议不小于 `10px`，随线宽线性关联。
...
### 状态机与阈值
- 首击创建：工具从选择态切换到绘制态后，进入“首击创建”状态；首次点击优先走新增流程，超过拖动阈值 `3px` 开始创建，随后恢复正常命中。
- 空白点击保护：文字与序号在刚创建或双击进入编辑后，抑制下一次空白处的创建；切换工具或结束编辑后解除。
- 编辑态路由：进入编辑将事件路由到编辑组件；退出编辑刷新光标矩形并恢复选择态命中。
...
#### 尺寸模型补充
- 徽章尺寸：半径与内部数字字号由独立基准推导（与文字字号解耦）；缩放徽章不改变标签文字大小。
- 文字尺寸：标签文字大小由字号控制；在非编辑态调整“文字大小”不影响徽章半径。

#### 编辑写回规则
- 编辑结束合并文字属性（字号、粗体、字体、颜色、底色）为查看态标签，保留徽章状态与位置。
- 占位态无标签时，编辑后生成标签并建立连接线；已有标签时更新属性与位置。

#### 连线几何裁剪
- 连线终点按标签矩形外缘投影裁剪：从徽章中心指向标签中心，命中矩形边界的外缘点作为绘制终点，避免穿透与遮挡。
...
### 快捷操作补充
- 方向键移动：基础 1px；`Shift` 加速为 10px。
- Option+方向键：对选区尺寸进行微调与缩放，遵循右下角为默认扩展方向的几何约定。
- 编辑态文本：`Cmd+A` 选中全部内容；确认与退出遵循全局规则。
...
### 光标矩形刷新策略
- 在进入/退出编辑态、工具切换、选中变化时刷新光标矩形；选择态注册控制点与拖拽区域手型光标，编辑态仅注册文本 I 光标。
...
### 操作与命令路由
- 动作统一通过命令总线路由（撤销、重做、删除、保存、复制、关闭、Pin、OCR、滚动截屏）。
- 工具栏与 SelectionView 层负责收集用户输入与手势，命令总线负责执行与状态管理，保持视图与逻辑解耦。
