...
*   **关键特性**：
    ...
    *   **属性面板**：
        *   **布局**：紧凑的 3 栏布局（大小/透明度 | 颜色选择 | 样式选项），右侧区宽度按内容自适应，避免遮盖左/中区。
        ...
        *   **数值标签**：大小/透明度数值按最大字符串测量（形状 `20px`、文字/序号 `100pt`），滑块宽度自适应，确保不被遮盖。
    *   **隐私保护**：马赛克/高斯模糊工具（用于遮挡敏感信息）。
    *   **文本工具**：添加文字注释。
        *   **中文输入法优化**：合成阶段按预计文本实时扩宽，禁用容器宽度换行，编辑态与保存后的尺寸一致。
    *   **撤销/重做**：支持编辑操作的回退。
...
*   **关键特性**：
    ...
    *   **防误触**：
        ...
        *   **选中取消**：当已有标注被选中时，点击空白处仅**取消选中**并隐藏属性面板。
        *   **文字/序号点击保护**：仅在刚创建后且未切换工具时抑制下一次空白点击创建；一旦在工具栏切换到任意工具，立即允许创建；仅调整属性不会解除保护。
